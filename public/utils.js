...
function processShifts(shifts) {
	return shifts.sort((a, b) => a.date - b.date).slice(0, 24);
}
...
function editDate(date, {
	...
	outputDate.setHours(hours, minutes, seconds, ms);

	return outputDate;
}

function dateToMilitaryTime(input) {
	const date = new Date(input);

	return date.toTimeString().slice(0, 5).replace(":", "");
}

function dateToMilitaryDate(input) {
	const date = new Date(input);

	const dateStr = date.toDateString();
	const monthStr = dateStr.slice(4, 7).toUpperCase();
	const dateNumStr = dateStr.slice(8, 10);
...
function dateValueToDate(value) {
	...
	return new Date(`${dateComponents[1]}/${dateComponents[2]}/${dateComponents[0]}`);
}
